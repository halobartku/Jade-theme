...
const ShopTheLook = () => {
  ...
  async function init(sectionId) {
    ...
    sections.forEach((section) => {
      ...
      setEventBusListeners(section.id);
      setEventListeners(section.id);
    });
    initSliders();
    initPointsListener();
  }
  function setEventBusListeners(sectionId) {
    sectionComponents.forEach(({ lookSlides, pointButtonsMobile }) => {
      lookSlides.forEach((slide) => {
        const blockIndex = +slide.dataset.blockIndex;
        window.themeCore.EventBus.listen(["EscEvent:on", `Overlay:shopTheLookDrawerProducts-${sectionId}-${blockIndex}:close`, `Toggle:shopTheLookDrawerProducts-${sectionId}-${blockIndex}:close`], () => {
  ...
  function setEventListeners(sectionId) {
    AFTER_MEDIUM_SCREEN.addEventListener("change", (event) => handleScreenChange(event, sectionId));
  ...
  function handleScreenChange(event, sectionId) {
    ...
    if (isDesktop) {
      window.themeCore.EventBus.emit(`Toggle:shopTheLookDrawerProducts-${sectionId}-${blockIndex}:close`);
      removeActiveClasses(pointButtonsMobile);
    } else {
  ...
  function togglePointButtonMobile(pointButton) {
    ...
    const closestProductsSlider = (_a = sectionComponents.find(
      (component) => component.productsSliders.some((slider) => +slider.dataset.blockIndex === blockIndex && component.section === pointButton.closest(selectors.section))
